...
export function renderOracleView(state) {
  ...
  const posture = stage === "question" ? "候" : stage === "ready" ? "契" : "解";
    ...
    <main class="shell">
      <header class="master-card stage-${stage}">
        <div class="portrait" role="img" aria-label="中式老卦师墨衡，当前仪态：${phase}">
         ...
         <div><p class="eyebrow">墨衡小卦 · ${phase}</p><h1>能闲聊，也能有据解卦</h1><p>普通问题直接聊；想占卜时再明确起卦，经传解释会显示检索来源。</p><span class="status ${state.cloud ? "online" : ""}">${cloudLabel}</span><small class="knowledge-status">${knowledgeLabel}</small></div>
      </header>
      ...
      <section class="controls">
        ...
        ${stage === "reading" && state.cloud ? `<p class="rag-invitation">现在可自由追问：原文、动爻、上下卦关系、不同理解，或它如何映照你的原问。墨衡会显示本轮检索到的经传依据。</p>` : ""}
        <form>
...
function evidenceDetails(evidence) {
  ...
  return `<details class="rag-evidence"><summary>本答检索依据 · ${evidence.length} 条</summary><ol>${evidence.map((item) => `<li><a href="${safeUrl(item.sourceUrl)}" target="_blank" rel="noreferrer">${escapeHtml(item.title)}</a><small>${escapeHtml(item.id)} · ${escapeHtml(item.layer)}</small><blockquote>${escapeHtml(item.excerpt)}</blockquote></li>`).join("")}</ol></details>`;
}
