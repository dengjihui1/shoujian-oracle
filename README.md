...
![版本](https://img.shields.io/badge/version-0.56.0-8e332a)
...
9. 最近 24 条已完成对话、当前原问和程序卦象保存在当前浏览器本机；刷新后恢复同一阶段与同一卦，重启服务不会清掉浏览器记录；“清空记录并重新开始”会删除它，每次最多发送最近 16 条且不超过约 6000 字符；
10. 可选启用 Google Cloud Speech-to-Text v1 流式转写，实时显示临时与定稿文字；未启用时使用浏览器增量识别，失败后切换本地停顿检测与 Gemini 整句转写。手动录音仍可选；
...
20. 支持独立“自动语音对话”：用户明确开启后，Google Cloud 或浏览器在线识别可显示增量转写；失败时自动切换可用的识别方式，定稿后送问；回答首个完整短句进入朗读，朗读时暂停收音，并可“打断并说话”。
...
经传数据另从中文维基文库公开来源建立独立知识包，不复制主项目住宅知识。语音 API 的真实边界、Google Cloud 配置和 RAG 构建全过程见 [语音 API 与 RAG 架构说明](docs/VOICE_RAG_ARCHITECTURE.md)；从上游 Cactus 提取了什么、拒绝照搬什么见 [Cactus 模块拆解](docs/CACTUS_MODULE_EXTRACTION.md)；完整映射见 [组件抽取与流程图](docs/COMPONENT_MAP.md)，逐模块说明见 [守简模块池](docs/MODULE_POOL.md)及[逐模块工程选择复核](docs/ENGINEERING_REVIEW_2026-09-24.md)，最新迭代见 [0.56.0 问心空间与交互重构](docs/RELEASE_0.56.0.md)，生产审计与待验收项见 [生产审计台账](docs/PRODUCTION_AUDIT_2026-09-24.md)。
...
不要把真实密钥写进前端、提交到 GitHub，或发送到聊天中。Google Cloud 实时转写需要另建 Cloud 项目并启用 Speech-to-Text API，步骤见 [实时转写启用指南](docs/GOOGLE_CLOUD_STT_SETUP.md)；其他配置见 [API 配置指南](docs/API_SETUP.md)。

已启动云端服务时，可运行 `npm run smoke:voice-browser`。它用云端 TTS 生成一段测试语音，在独立 Chromium 中通过合成麦克风交给真实 `MediaRecorder`，再调用 `/api/transcribe`；会消耗语音 API 额度，不接触电脑麦克风，也不会记录密钥。该测试检验浏览器录音与服务端链路，不能代替真实设备的噪声和回声验收。

云端链路为：

```text
麦克风 → 浏览器实时转写（可用时）→ 可编辑文字
自动语音对话 → 可选 Google Cloud 流式转写 → 浏览器增量识别 → 停顿检测与 Gemini 整句兜底 → 自动提交
浏览器增量识别网络失败 → 自动停顿收音 → Gemini 3.5 Transcribe 单请求 → 自动送问；手动录音仍可选
...
- `server/rate-limiter.mjs` / `redis-rate-limiter.mjs`：本地内存限流、生产 Redis 原子共享窗口与 HMAC 客户端键；
