...
![版本](https://img.shields.io/badge/version-0.10.0-8e332a)
...
7. Gemini 回答通过 SSE 真流式传输，再以字符级动画持续写进同一条气泡；若代理或供应商在半途断流，服务端会自动恢复完整回答并原位替换半截内容；
...
16. 没有 API 密钥时自动退回本地有限对话，起卦仍然可用。
...
经传数据另从中文维基文库公开来源建立独立知识包，不复制主项目住宅知识。完整映射见 [组件抽取与流程图](docs/COMPONENT_MAP.md)，逐模块输入输出、依赖、降级、测试和练习见 [守简模块池](docs/MODULE_POOL.md)，阅读顺序见 [代码学习指南](docs/LEARNING_GUIDE.md)，本轮自动化与真实 Gemini 验收见 [0.10.0 质量基线](docs/QUALITY_BASELINE.md)。
...
普通文字 → Gemini 3.5 Flash SSE（繁忙时自动尝试备用模型；半途断流自动恢复完整回答）→ 字符级呈现的墨衡自然回答
