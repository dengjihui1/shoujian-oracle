...
test("session records ASR final, first token and first audio latency", async () => {
  ...
  clock = 1_260;
  session.markSpeechState("playing");
  assert.equal(session.snapshot.metrics.asrFinalMs, 580);
  assert.equal(session.snapshot.metrics.firstTokenMs, 240);
  assert.equal(session.snapshot.metrics.firstAudioMs, 580);
  session.markSpeechState("idle");
  finishTurn();
  await tick();
  session.stop();
