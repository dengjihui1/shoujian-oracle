...
- 正常路径：未配置 `REDIS_URL` 时保持零配置本地运行；生产 Compose 默认启用私网 Redis，多个应用实例共享同一窗口并使用 Redis 时钟计算窗口。
...
0.47.0 的录音会话隔离见 [0.47.0 发布记录](RELEASE_0.47.0.md)。P07 的音频块、录音器、媒体流和自动停止计时器绑定到本轮，旧 `stop` 事件不会污染或停止新录音。

0.48.0 的供应商流清理见 [0.48.0 发布记录](RELEASE_0.48.0.md)。S03 与 S06 在流事件格式异常时主动取消读取器，避免仅释放锁却留下未消费的上游响应。
...
0.52.0 的浏览器录音复验见 [0.52.0 发布记录](RELEASE_0.52.0.md)。P01 监听录音器自行结束或报错，自动进入转写或解除输入占用，并用逐轮代数丢弃旧实时识别的迟到临时文字与完成结果。合成麦克风 + 真 `MediaRecorder` + 真云端接口已在 Chromium 回环通过；当前 Windows Playwright WebKit 缺少媒体 API，不能把模拟录音回归写成 Safari 实机通过。
