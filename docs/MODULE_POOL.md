...
当前自动化、真实云端链路和浏览器人工验收结果见 [0.27.0 历史质量基线](QUALITY_BASELINE.md)与 [0.28.0 全方位评估](ASSESSMENT_2026-09-23.md)。语音队列现在只在播放器触发开声事件后进入 `playing`；服务端只根据六爻重算本卦、动爻和之卦，不再信任请求中的展示名称。
