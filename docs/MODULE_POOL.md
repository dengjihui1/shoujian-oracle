...
- 依赖：D01。
- 正常路径：普通专业话题继续聊天；卦后只有“本卦、动爻、之卦、原问”等追问强制绑定卦证据；投资、医疗、法律、生意等起卦照常解释并追加领域参考说明；现实危机给立即行动步骤。
...
- 正常路径：聊天首模型过载且尚未产出时尝试备用；已有分片后由 S01 通过非流式接口恢复；短音频优先内联。
...
当前自动化、真实 Gemini 和浏览器人工验收结果见 [0.10.0 质量基线](QUALITY_BASELINE.md)。
