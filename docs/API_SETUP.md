...
本文对应项目 `0.6.1`。模型和免费额度会变化；下面的模型 ID 已在 2026-09-20 通过当前账号的 Gemini Models API 与真实请求核验。
...
8. 起卦后输入任意相关追问，回答旁显示“墨衡 · RAG”，并能展开本卦、实际动爻和之卦来源；
9. 支持浏览器实时识别时，点击“实时语音输入”，说话期间文字应逐步进入输入框；停止后仍可修改再提交；
10. 不支持实时识别时，页面显示“按下说话”，停止后使用 Gemini 单请求内联音频转写，失败时才退回 Files API；
11. 打开“语音回答”再提问：文字完成后输入框必须立即恢复，语音在后台生成；此时可以继续提问；
12. 回答期间应出现“停止回答”，转写期间应出现“取消转写”，两者均可恢复可输入状态。
...
服务端每轮把当前 `Asia/Shanghai` 时钟写入系统提示词。最近 24 条完成的对话仅保存在当前浏览器 `localStorage`，刷新后恢复；每次请求最多发送最近 16 条，错误回答和未完成流不会保存。“清除本机记忆”可立即删除这些记录，服务端不建立用户档案。
