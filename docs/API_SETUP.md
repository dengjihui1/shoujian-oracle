...
本文对应项目 `0.21.0`。模型、音色、免费额度和控制台界面会变化；下面的 Gemini 模型 ID 已在 2026-09-22 通过当前账号的真实状态与请求核验，Google Cloud TTS 适配器已通过自动测试，但仍需用你自己的 Cloud 凭证完成真实调用验收。
...
Docker、正式域名、Caddy 自动 HTTPS、健康检查、结构化脱敏日志和单实例限流边界见 [生产部署指南](DEPLOYMENT.md)。
