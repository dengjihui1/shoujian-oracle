...
本文对应项目 `0.10.0`。模型和免费额度会变化；下面的模型 ID 已在 2026-09-21 通过当前账号的真实状态与请求核验。
...
| 能力 | 默认模型 | 接口 | 本项目职责 |
| --- | --- | --- | --- |
| 自由对话 | `gemini-3.5-flash`，备用 `gemini-3.6-flash` | `streamGenerateContent?alt=sse` | 普通闲聊、墨衡人格与卦象解释；SSE 真流式输出；首个分片前失败时换备用模型，已有分片后断流时恢复完整答案并原位替换 |
...
- 回答已经出现但中途网络抖动：0.10.0 会用同一系统约束和检索证据恢复完整回答，再通过 `replace` 事件原位替换半截文本；若恢复请求也失败，才显示稳定错误提示。
