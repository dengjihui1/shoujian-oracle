...
- 服务端先做长度、类型和问题边界校验；
- 服务端只接收六个合法爻值，再调用与浏览器相同的纯函数重算本卦、动爻和之卦；请求里附带的卦名不作为可信输入；
...
11. 阅读 `request-context.mjs`、`observability.mjs`、`redis-rate-limiter.mjs` 与 `docs/DEPLOYMENT.md`，理解为什么代理信任必须显式开启、日志不应含用户正文，以及 Redis 键仍要先做带盐 HMAC。
12. 阅读 `voice-performance.js`，用 `docs/DEVICE_ACCEPTANCE.md` 在真实设备完成至少 10 轮，确认 P50 / P95 报告不含录音或转写文字。
